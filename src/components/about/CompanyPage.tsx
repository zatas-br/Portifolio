...
function Tag({ label }: { label: string }) {
  return (
    <span className="font-serif italic font-normal text-[clamp(13px,1.1vw,20px)] text-[#0D47A1] border-[1.5px] border-[#0D47A1] rounded-full px-4 py-1 inline-flex items-center leading-[1.5] whitespace-nowrap">
      {label}
    </span>
  );
}
    ...
    <div className="bg-[#ECEFF1] font-sans min-h-screen overflow-x-hidden">
      ...
      <section className="relative w-full">
        <img
          ...
          className="w-full h-auto block max-h-[calc(100vh-80px)] object-cover object-top"
        />
      </section>
      ...
      <section className="bg-[#ECEFF1] pt-[5.5vw] pb-[4vw] pl-[12.7vw] pr-[5vw] max-[900px]:px-6 max-[900px]:pt-10 max-[900px]:pb-8">
        ...
        <div className="flex flex-col gap-[clamp(8px,1vw,14px)] max-w-[41.7vw] max-[900px]:max-w-full">
          {cfg.commitmentItems.map((item, i) => (
            <p key={i} className="font-light text-[clamp(12px,1.1vw,16px)] text-[#263238] leading-[1.75] m-0 mb-[2px]">
              {item}
            </p>
          ))}
          <p className="font-light text-[clamp(12px,1.1vw,16px)] text-[#263238] leading-[1.75] m-0 mt-[14px]">
      ...
      <section className="bg-[#ECEFF1] px-[5.5vw] pb-[6vw] pt-[3vw] max-[900px]:px-6 max-[900px]:pb-12">
        <div className="w-full max-w-[1421px] mx-auto aspect-[1433/546] bg-[#263238] rounded-[clamp(16px,1.5vw,24px)] overflow-hidden grid grid-cols-1 md:grid-cols-[51%_49%] shadow-[0_16px_48px_rgba(0,0,0,0.2)] max-[900px]:aspect-auto">
          ...
          <div className="px-[clamp(24px,3vw,48px)] py-[clamp(24px,3.5vw,56px)] flex flex-col justify-between h-full box-border">
            ...
            <div className="flex flex-col gap-[clamp(6px,0.7vw,10px)] flex-1">
              {cfg.historyParagraphs.map((p, i) => (
                <p key={i} className="font-light text-[clamp(11px,1.3vw,18px)] text-white leading-[1.6] m-0">
                  {p}
                </p>
              ))}
            </div>
