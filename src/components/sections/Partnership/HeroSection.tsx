...
const HeroSection = () => {
  const t = useTranslations('PartnershipsPage.hero');
      ...
      <div className="w-full flex flex-col items-center gap-5 mt-10 md:pb-12">
        ...
        <div className="hidden md:flex flex-row flex-wrap justify-center items-center gap-10 md:gap-16 opacity-80 grayscale">
          {PARTNERSHIP_LOGOS.map((logo, index) => (
            <div key={index} className="w-20 flex justify-center">
              <img src={logo.src} alt={logo.name} className="w-full h-auto object-contain" />
            </div>
          ))}
        </div>
        ...
        <div className="md:hidden opacity-80 grayscale w-full max-w-sm flex flex-col gap-5">
          <div className="grid grid-cols-4 gap-x-6">
            {PARTNERSHIP_LOGOS.slice(0, 4).map((logo, index) => (
              <div key={index} className="flex justify-center items-center">
                <img src={logo.src} alt={logo.name} className="w-14 h-auto object-contain" />
              </div>
            ))}
          </div>
          <div className="flex justify-center gap-6">
            {PARTNERSHIP_LOGOS.slice(4).map((logo, index) => (
              <div key={index} className="flex justify-center items-center">
                <img src={logo.src} alt={logo.name} className="w-14 h-auto object-contain" />
              </div>
            ))}
